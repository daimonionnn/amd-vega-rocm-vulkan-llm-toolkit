...
- [ ] **Can gfx900 be backported into a newer PyTorch wheel?** Only `torch 2.7.0+rocm6.3` ships gfx900 kernels — it carries **128 gfx900 files** in `torch/lib/rocblas/library/`, the same set this repo backports, while `torch 2.11.0+rocm7.2` carries **zero**. Those are injectable, exactly as for llama.cpp. The gate is whether PyTorch's own ATen kernels were compiled for gfx900; if not, injecting rocBLAS files achieves nothing. **Do not try to settle that by inspecting `libtorch_hip.so`** — attempted 2026-09-11 and it does not work: neither `amdgcn-amd-amdhsa--gfx*` triples nor `__CLANG_OFFLOAD_BUNDLE__` markers appear in that library even in the wheel that demonstrably runs on gfx900, and a bare `gfx900` match hits an architecture *name table* that also lists gfx800/gfx803/gfx901. The first attempt produced a false negative, caught only by running the same check against the known-good wheel. Settle it empirically instead: install `torch 2.11+rocm7.2` in a container, drop the gfx900 rocBLAS files in, and run [`build/pytorch-smoke.py`](build/pytorch-smoke.py) — it compares against CPU references, so a missing kernel cannot pass quietly. **Not urgent:** `torch 2.7.0+rocm6.3` works and the system ROCm version is irrelevant to it, since the wheel bundles its own
