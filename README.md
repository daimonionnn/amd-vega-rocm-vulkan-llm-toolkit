...
- [ ] **TheRock can build ROCm natively for `gfx90c` — no override, no backport.** Upstream has a first-class target: `therock_add_amdgpu_target(gfx90c "AMD Renoir/Lucienne/Cezanne iGPU" FAMILY igpu-all gfx90c-igpu)`, with the same exclusion list as gfx900 (hipBLASLt, hipSPARSELt, composable_kernel, rocWMMA, hipTensor, rocprofiler-compute — each with a linked upstream issue). It would retire `HSA_OVERRIDE_GFX_VERSION=9.0.0` and the Tensile copy at once. **Nobody ships it:** AMD's nightly index publishes gfx900, gfx906, gfx908, gfx90a, gfx90X-dcgpu, gfx94X, gfx950 and the RDNA families, while `gfx90c`, `gfx90c-igpu` and `igpu-all` all return 404 (checked 2026-09-12) — a defined target is not a built one, so this means building ROCm from source. **Expect no speed from it:** 7.14 against 7.2 here was noise in 11 of 12 cells and 8 % *worse* in the twelfth, and a gfx90c rocBLAS would need Tensile kernels of its own, untuned for 8 CUs. The gain is architectural — no substitution step, and a road to newer ROCm and PyTorch — not throughput. **If it is ever done:** a containerised overnight build with `-DTHEROCK_AMDGPU_FAMILIES=gfx90c-igpu`, first milestone `rocminfo` and `test-backend-ops`, benchmarks only after that. **Cheaper first:** ask [mixa3607](https://github.com/mixa3607/ML-gfx906) whether their pipeline can emit gfx90c — one question instead of a night of compiling. **Not attempted here** as of 2026-09-12
- [ ] **Use the ML-gfx906 pipeline as the template for the whole ML stack.** [mixa3607/ML-gfx906](https://github.com/mixa3607/ML-gfx906) is not directly usable here — it targets gfx906, and its recent commits are a Vega20 metrics exporter — but it is standing proof that the full chain works for a *dropped* gfx9 architecture: a TheRock-built ROCm 7.14, then **PyTorch 2.13.0**, ComfyUI, vLLM and llama.cpp built against it, published as Docker images and an APT repo (`docker.io/mixa3607/pytorch-gfx906:v2.13.0-rocm-7.14`). That is the answer to being pinned to `torch 2.7.0+rocm6.3`, which is the only stock wheel with gfx900 kernels. It is split into per-component directories (`rocm/`, `rocm-tensile/`, `pytorch/`, `comfyui/`, `vllm-v2/`), so stages could in principle be retargeted independently rather than as one monolith — but **whether it can be pointed at another architecture at all is unverified here**: their ROCm workflow carries the architecture inside a build string (`rocm_build: "7.14.0-gfx906+20260802001858"`) with no `ROCM_ARCH` or `GPU_TARGETS` input, and the retargeting logic, if there is any, sits in a build script not read here. An earlier version of this entry asserted the pipeline is parameterised by `ROCM_ARCH`; that was never checked. `rocm-tensile/` is worth reading on its own: it rebuilds Tensile kernels for a dropped arch against 6.3.3 / 6.4.4 / 7.0.0 / 7.0.2, which is the general form of what `patches/` and the 6.3.4 file copy do by hand here. Note they exclude hipBLASLt (see their `HIPBLASLT-GFX906.md`), which breaks INT8 `torch._int_mm` — the same exclusion TheRock applies to gfx900 and gfx90c
