...
- [x] **ROCm 7.14 made to work on this APU (2026-09-11)** — AMD's gfx900 wheels segfault in `hsa_init()` → `GpuAgent::InitDma()`, but a *different* 7.14 build of the same source (`mixa3607/rocm-gfx906:7.14-complete`) enumerates this GPU fine, which proves the crash is that wheel build rather than the 7.14 source. Combining their runtime with the 182 gfx900 Tensile files out of AMD's wheel gives a working ROCm 7.14: `test-backend-ops -o FLASH_ATTN_EXT` **2959/2959**, and performance within noise of ROCm 7.2 on three of four cells (gemma decode @16K is ~9 % slower and wants a proper repeat). [`build/Dockerfile.rocm714-mixa-gfx900`](build/Dockerfile.rocm714-mixa-gfx900), results in [bench/results/2026-09-11-rocm714-working.md](bench/results/2026-09-11-rocm714-working.md). **The 6.3.4 backport into ROCm 7.2 stays the default** — tested longer and currently a little faster — but it is no longer the only option
