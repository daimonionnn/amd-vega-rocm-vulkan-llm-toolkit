...
- [x] **AMD's prebuilt `rocm-sdk-libraries-gfx900` tried and rejected (2026-09-11)** — the SDK is real (targets `gfx900`, 182 Tensile files against our 128) and llama.cpp **builds against it cleanly** with `patches/0001`, but the ROCm 7.14 runtime **segfaults during GPU enumeration on this APU**: `rocminfo`, `hipGetDeviceCount` and 1 of `rocm-sdk test`'s 26 tests all die with SIGSEGV. Not the override — it segfaults identically with `HSA_OVERRIDE_GFX_VERSION` unset. Same wall as June 2026's modular `amdrocm-core`, failing less gracefully. **The Tensile backport into classic ROCm 7.2 stays.** Says nothing about *discrete* gfx900, where issue #1 reports 7.14 working. Full write-up and the two traps found on the way in [bench/results/2026-09-11-rocm714-sdk-gfx900.md](bench/results/2026-09-11-rocm714-sdk-gfx900.md); the recipe is kept in [build/Dockerfile.rocm714-sdk-vega](build/Dockerfile.rocm714-sdk-vega) in case a later runtime fixes it
...
- [ ] **Can gfx900 be backported into a ROCm 7 PyTorch wheel?** Only `torch 2.7.0+rocm6.3` ships real gfx900 kernels (`rocblas/library/Kernels.so-000-gfx900.hsaco` + `TensileLibrary_lazy_gfx900.dat`); 6.4, 7.0, 7.1 and 7.2 wheels carry only leftover MIOpen `gfx900_56.db.txt` tuning files and no kernels. So ML work is pinned to a two-generation-old ROCm — worth escaping if 6.3 turns out slow, since on llama.cpp ROCm 7.2 beat 6.2.4 by **+7.7 % prefill and +6.8 % decode on the 35B, +17 % decode on gemma** (2026-05-14, different codebase, so suggestive only). The wheel stores rocBLAS in exactly the layout this repo's Tensile backport targets, so copying the gfx900 files in is the same trick. **Check this first, it is the gate:** PyTorch's own ATen kernels are fat-binary-embedded in `torch/lib/libtorch_hip.so`, and if that was not compiled for gfx900 then backporting rocBLAS alone achieves nothing and the only route is building PyTorch from source. Same method as the package survey — `strings -a libtorch_hip.so | grep -oE 'amdgcn-amd-amdhsa--gfx[0-9a-z:+-]+'`. Do this only after the 6.3 smoke test establishes a baseline to compare against
