...
- **AMD's modular packages: an APU problem only.** Their ROCr rejects
  \`HSA_OVERRIDE_GFX_VERSION\`, so gfx90c cannot present itself as gfx900 there.
  A discrete Vega 10 needs no override at all, and issue #1 reports these files
  working on one under modular rocBLAS 7.14.1.
